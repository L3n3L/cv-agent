...
本节是对前文中间修复记录的最终收口。前文 13.1 记录的“按 runId 分组”是过渡实现；本轮最终实现以 `turnId` 作为用户回合归属键，并由统一 reducer 完成消息、workflow 和实时 delta 的归并，不能再把前文过渡实现当作当前 renderer 契约。
...
- Backend：`npm.cmd test`，77/77 通过；其中包含多轮时间排序、孤立 workflow 抑制、空生命周期不拆组和前端契约回归。
- Frontend：`npm.cmd run build`，TypeScript 检查和 Vite build 均通过。
- 浏览器：真实打开 Agent 并检查最新构建；多轮 session 中用户消息、对应 workflow 和 Agent 消息按真实时间顺序排列，不再把旧 workflow 追加到页面底部；工具组默认折叠，输入框保持可用，控制台无 error/warning。
- 当前浏览器模型调用未使用真实 API key，因此本轮浏览器没有宣称“真实模型成功执行”通过；暂停、测量、自动 continuation、完整工具链由后端集成测试覆盖。
- 本轮没有依赖删除旧 session 来掩盖历史数据问题；保留的旧 session 仍可作为兼容回归样本，新 session 使用 `messageSequence`，前端统一 reducer 同时兼容带时间/序号的新数据和无序号的历史消息。
...
本轮浏览器验收还暴露了一个开发环境约束：不能同时启动多个指向同一 `.cvagent` 数据目录的后端实例。旧实例会持有自己的 session 内存快照，同时把事件写回共享文件，新实例再读取时就可能出现“页面显示了另一条会话的工具轨迹”。验收时已停掉旧的 3180/3192 服务，仅保留 3181/3193 单实例组合；本轮顺序验收保留历史 session 作为兼容样本，没有依赖清理数据来规避问题。后续应在启动脚本或服务启动检查中增加“数据目录锁/端口-实例绑定”，避免开发环境再次串线。

在上述三项完成前，本 spec 不标记为“最终完成”；当前状态是“数据流主链已收口，生产级浏览器全链路仍待带凭据验收”。本轮已完成的是文档要求的 P0 渲染源收口与多轮顺序修复，不把它误报成真实模型全链路已完成。
