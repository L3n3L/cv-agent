...
test('React A4 pane preserves the preview contract and post-mount status sync', async () => {
  ...
  assert.match(app, /function currentPreviewState\(\)/)
  assert.match(app, /status: 'awaiting_render'/)
  assert.match(app, /preview: currentPreviewState\(\)/)
  assert.match(app, /liveState\.loading = false[\s\S]*?renderRoute\('workbench'\)/)
  assert.doesNotMatch(app, /activeSessionId/)
  assert.match(app, /frame\.closest\('\.direct-preview-stage'\)\) return/)
  assert.match(app, /window\.addEventListener\('cvagent:a4-preview-changed'/)
  ...
  assert.match(pane, /export type PreviewState/)
  assert.match(pane, /preview\.url \?/)
  assert.match(pane, /data-testid="preview-empty"/)
