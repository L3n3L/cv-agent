...
import { resolveWorkspacePath, writeWorkspaceFile } from './workspace.js'
...
const VALID_PRESETS = PRESETS.map((preset) => assertTemplateSpec({
  ...
  metadata: {
    ...(preset.metadata || {}),
    generatedBy: 'cvagent-built-in-template',
    family: preset.family || 'campus-clear',
    lineageId: preset.id,
...
export function templateMetadata(template, { revision, sourceTemplateId, immutable } = {}) {
  const source = template?.metadata && typeof template.metadata === 'object' ? template.metadata : {}
  const id = String(template?.id || '')
  return {
    ...source,
    lineageId: String(source.lineageId || source.sourceTemplateId || id),
    sourceTemplateId: sourceTemplateId === undefined ? (source.sourceTemplateId || null) : sourceTemplateId,
    revision: Number.isInteger(revision) && revision > 0 ? revision : (Number.isInteger(source.revision) && source.revision > 0 ? source.revision : 1),
    immutable: immutable === undefined ? Boolean(source.immutable) : Boolean(immutable),
  }
}
...
function rewriteTemplateCssScope(css, sourceId, targetId) {
  const value = typeof css === 'string' ? css : ''
  if (!value || !sourceId || !targetId || sourceId === targetId) return value
  const escapedSourceId = String(sourceId).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const quoted = value.replace(
    new RegExp(`(data-template-id\\s*=\\s*)(["'])${escapedSourceId}\\2`, 'g'),
    `$1$2${targetId}$2`,
  )
  return quoted.replace(
    new RegExp(`(data-template-id\\s*=\\s*)${escapedSourceId}(?=\\s*\\])`, 'g'),
    `$1${targetId}`,
  )
}
...
async function readOptionalTemplateCss(root, id) {
  const cssPath = builtInTemplate(id)
    ...
    : resolveWorkspacePath(root, `templates/${id}.css`).abs
  try {
...
export async function listAvailableTemplates(root) {
  ...
  const dir = resolveWorkspacePath(root, 'templates').abs
  let entries = []
  try {
...
export async function loadTemplate(root, id) {
  ...
  const { abs } = resolveWorkspacePath(root, `templates/${id}.json`)
  const parsed = JSON.parse(await fs.readFile(abs, 'utf8'))
  const result = validateTemplateSpec(parsed)
...
export async function saveTemplate(root, input, { replaceExisting = false, sourceTemplateId } = {}) {
  ...
  assertCustomTemplateId(result.value.id)
  const { abs, rel } = resolveWorkspacePath(root, `templates/${result.value.id}.json`)
  const cssAbs = resolveWorkspacePath(root, `templates/${result.value.id}.css`).abs
  ...
  if (previous && !replaceExisting) {
    const conflict = new Error(`template already exists: ${result.value.id}; use template_copy for a new revision or explicitly confirm replacement`)
    conflict.code = 'TEMPLATE_CONFLICT'
    throw conflict
  }
  let previousCss = ''
  try {
  ...
  const cssBeforeScopeRewrite = hasTemplateCss ? result.value.templateCss : previousCss
  const nextCss = sourceTemplateId && sourceTemplateId !== result.value.id
    ? rewriteTemplateCssScope(cssBeforeScopeRewrite, sourceTemplateId, result.value.id)
  ...
  const metadata = templateMetadata(result.value, {
    revision: nextRevision,
    sourceTemplateId: sourceTemplateId === undefined ? result.value.metadata?.sourceTemplateId : sourceTemplateId,
    immutable: false,
  })
  ...
  if (nextCss) {
    await writeWorkspaceFile(root, `templates/${result.value.id}.css`, nextCss)
...
export async function copyTemplate(root, sourceId, newId, name) {
  ...
  const next = {
    ...
    metadata: templateMetadata(source, { sourceTemplateId: sourceId, revision: 1, immutable: false }),
  }
  const saved = await saveTemplate(root, next, { sourceTemplateId: sourceId })
  return { ...saved, createdAsCopy: true, sourceTemplateId: sourceId, lineageId: saved.template?.metadata?.lineageId || sourceId }
}
...
export async function restoreLatestTemplate(root, id) {
  ...
  return restoreTemplateVersion(root, id, versions[0].id)
}

export async function restoreTemplateVersion(root, id, versionId) {
  if (!/^[a-zA-Z0-9_-]+$/.test(String(versionId || ''))) throw new Error('invalid template version id')
  const versions = await listTemplateVersions(root, id)
