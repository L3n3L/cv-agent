...
5. 模板结构/CSS 变化必须基于 template_copy 或用户确认的 template_save；仅字号、间距、颜色、分隔线和图标微调使用 presentation_update。未知图标必须先 icon_list，不得猜 token。
...
7. presentation_suggest 只会根据当前真实测量提出受限建议，不会自行修改。resume_save_version 仅能在 finalize 通过且用户明确确认后调用；保存版本可记录目标岗位、公司和 JD 相对路径。
`
