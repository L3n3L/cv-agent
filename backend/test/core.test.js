...
test('migrated template catalog and presentation tuning are first-class tools', async () => {
  ...
  try {
    ...
    assert.equal(generated.template.layoutSpec.ir.type, 'stack')
    assert.ok(generated.template.layoutSpec.blocks.some((block) => block.type === 'timeline'))
    ...
    const generatedRender = await tools.find((item) => item.name === 'resume_render').invoke({})
    const generatedHtml = await fs.readFile(generatedRender.absolutePath, 'utf8')
    assert.match(generatedHtml, /data-module-type="timeline"/)
...
test('measurement callback verifies the exact rendered artifact', async () => {
  ...
  try {
    ...
    const firstMeasureResponse = await fetch(`http://127.0.0.1:${address.port}/api/agent/measure`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId: run.sessionId, renderId: run.context.renderId, pageCount: 2, occupancy: [0.5, 0.2], overflow: false }) })
    const firstMeasured = await firstMeasureResponse.json()
