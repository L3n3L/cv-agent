...
const PAGE_SPEC_FLOW_LAYOUTS = new Set(['stack', 'balanced-footer'])
...
export const COMPOSITION_PAGE_SPEC_DEFAULTS = Object.freeze({
  ...
  flow: { order: ['profile', 'summary', 'experience', 'projects', 'education', 'skills'], layout: 'stack', keepEntryTogether: true, avoidSectionOrphans: true },
...
export const TEMPLATE_DEFAULTS = Object.freeze({
  ...
  layout: {
    ...
    pageTarget: 1,
    density: 'standard',
    moduleOrder: ['profile', 'education', 'experience', 'projects', 'skills', 'awards'],
    sidebarRatio: 0.32,
    columnGap: 24,
  },
  typography: {
...
export function normalizeCompositionPageSpec(input = {}) {
  ...
  return {
    ...
    flow: {
      ...
      layout: PAGE_SPEC_FLOW_LAYOUTS.has(flow.layout) ? flow.layout : defaults.flow.layout,
...
export function validateCompositionPageSpec(input) {
  ...
  const rawFlow = raw.flow && typeof raw.flow === 'object' && !Array.isArray(raw.flow) ? raw.flow : {}
  const errors = []
  ...
  if (rawFlow.layout !== undefined && !PAGE_SPEC_FLOW_LAYOUTS.has(rawFlow.layout)) errors.push('composition.pageSpec.flow.layout is invalid')
  for (const key of ['top', 'right', 'bottom', 'left']) {
...
export function validateCssText(value, { kind = 'templateCss', maxLength = MAX_TEMPLATE_CSS_LENGTH } = {}) {
  ...
  for (const match of css.matchAll(CSS_URL_PATTERN)) {
    if (!safeCssUrl(match[2])) {
      errors.push(`${kind} url() may only use data:image/*, #fragment, or /api/asset resources`)
      break
    }
  }
